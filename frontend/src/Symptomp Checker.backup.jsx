...
const DiagnosisSearchPage = () => {
  ...
  // Enhanced symptoms with categories and related terms
  const commonSymptoms = [
    { text: 'Headache', category: 'Pain', related: ['migraine', 'tension', 'sinus'] },
    { text: 'Fever', category: 'General', related: ['high temperature', 'chills', 'sweating'] },
    { text: 'Cough', category: 'Respiratory', related: ['dry cough', 'chesty cough', 'persistent cough'] },
    { text: 'Fatigue', category: 'General', related: ['tiredness', 'lethargy', 'low energy'] },
    { text: 'Dizziness', category: 'Neurological', related: ['lightheaded', 'vertigo', 'unsteady'] },
    { text: 'Nausea', category: 'Digestive', related: ['queasy', 'upset stomach', 'vomiting'] },
    { text: 'Sore throat', category: 'ENT', related: ['throat pain', 'difficulty swallowing', 'hoarseness'] },
    { text: 'Shortness of breath', category: 'Respiratory', related: ['breathlessness', 'wheezing', 'chest tightness'] },
    { text: 'Chest pain', category: 'Cardiac', related: ['chest tightness', 'heartburn', 'pressure in chest'] },
    { text: 'Abdominal pain', category: 'Digestive', related: ['stomach ache', 'cramps', 'bloating'] },
    { text: 'Diarrhea', category: 'Digestive', related: ['loose stools', 'stomach upset', 'gastroenteritis'] },
    { text: 'Muscle aches', category: 'Musculoskeletal', related: ['body ache', 'soreness', 'pain in muscles'] },
    { text: 'Joint pain', category: 'Musculoskeletal', related: ['arthritis', 'stiffness', 'swelling'] },
    { text: 'Rash', category: 'Dermatological', related: ['skin irritation', 'hives', 'redness'] },
    { text: 'Loss of appetite', category: 'General', related: ['reduced hunger', 'weight loss', 'lack of interest in food'] },
    { text: 'Heartburn', category: 'Digestive', related: ['acid reflux', 'indigestion', 'GERD'] },
    { text: 'Back pain', category: 'Musculoskeletal', related: ['lower back pain', 'upper back pain', 'spinal pain'] },
    { text: 'Difficulty sleeping', category: 'General', related: ['insomnia', 'restless sleep', 'waking up frequently'] },
    { text: 'Frequent urination', category: 'Urinary', related: ['urinary frequency', 'urge to urinate', 'nocturia'] }
  ];
  
  // Get filtered symptoms based on user input
  const getFilteredSymptoms = (input) => {
    if (!input) return [];
    const inputLower = input.toLowerCase();
    return commonSymptoms.filter(symptom => 
      symptom.text.toLowerCase().includes(inputLower) ||
      symptom.related.some(term => term.includes(inputLower))
  ...
  const getConditionForSymptom = (symptom) => {
    ...
    // Enhanced conditions with more detailed information
    const conditions = {
      // Headache related
      'headache': {
        name: 'Tension Headache',
        description: 'A common headache often described as a constant ache or pressure around the head.',
        severity: 'mild',
        recommendations: [
        ...
        whenToSeeDoctor: 'If headaches are severe, persistent, or accompanied by other symptoms like vision changes or confusion'
      },
      'migraine': {
        name: 'Migraine',
        ...
        recommendations: [
          ...
          'Stay hydrated',
          'Consider migraine-specific medications if prescribed'
        ],
        whenToSeeDoctor: 'For frequent or severe migraines, or if they interfere with daily activities'
      },
      // Fever related
      'fever': {
        name: 'Viral Infection',
        description: 'Elevated body temperature often caused by infections.',
        ...
        whenToSeeDoctor: 'If fever is above 103°F (39.4°C), lasts more than 3 days, or is accompanied by severe symptoms'
      },
      // Add more conditions as needed
    };
    
    // Find matching condition or return a default
    const matchedCondition = Object.entries(conditions).find(([key]) => 
      symptomLower.includes(key) || key.includes(symptomLower)
    );
    
    // Default condition if no match found
    const defaultCondition = {
      ...
      whenToSeeDoctor: 'If symptoms persist for more than a few days or worsen'
    };
    
    return matchedCondition ? matchedCondition[1] : defaultCondition;

          ...
          <form onSubmit={handleSearch} className="space-y-6">
            ...
            <div className="mt-6">
              <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Popular Cities for Diagnosis Centers:
              </h4>
              <div className="flex flex-wrap gap-2">
                {['Hyderabad', 'Mumbai', 'Bangalore', 'Chennai', 'Kolkata', 'Delhi', 'Pune', 'Ahmedabad', 'Jaipur', 'Lucknow'].map((city) => (
                  <button
                    key={city}
                    type="button"
                    onClick={() => {
                      setSymptoms(prev => prev ? `${prev}, ${city} diagnosis centers` : `Find diagnosis centers in ${city}`);
                      inputRef.current?.focus();
                    }}
                    className="px-3 py-1.5 text-sm rounded-full bg-blue-50 hover:bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-300 transition-colors"
                  >
                    {city}
                  </button>
                ))}
